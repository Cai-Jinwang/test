...
# 提取文本和标签
train_texts = [tokenize_text(line.split('\t')[0]) for line in train_lines]
train_labels = [line.split('\t')[1].strip() for line in train_lines]
test_texts = [tokenize_text(line.split('\t')[0]) for line in test_lines]
test_labels = [line.split('\t')[1].strip() for line in test_lines]
dev_texts = [tokenize_text(line.split('\t')[0]) for line in dev_lines]
dev_labels = [line.split('\t')[1].strip() for line in dev_lines]
...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, text):
        ...
        embedded = self.dropout(self.embedding(text))  # embedded: [batch_size, seq_len, embedding_dim]
        ...
        return self.fc(hidden)  # 返回的形状为 [batch_size, output_dim]
